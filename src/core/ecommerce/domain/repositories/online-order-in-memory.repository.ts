...
import { IOnlineOrderRepository, OnlineOrderFilter, OnlineOrderSearchParams, OnlineOrderSearchResult } from '@core/ecommerce';
  ...
  implements IOnlineOrderRepository
{
  ...
  async findByClientId(clientId: string, storeId: string): Promise<OnlineOrder[]> {
    return this.items.filter(item => 
      item.client_id.id === clientId && item.store_id === storeId
    );
  }

  async findByStatus(status: string, storeId: string): Promise<OnlineOrder[]> {
    return this.items.filter(item => 
  ...
  async findHighValueOrders(storeId: string, minValue: number): Promise<OnlineOrder[]> {
    const minValueMoney = new Money(minValue);
    return this.items.filter(item => 
      item.store_id === storeId && (item.total.isGreaterThan(minValueMoney) || item.total.isEqualTo(minValueMoney))
    );
  }
  ...
  async findOrdersForDelivery(storeId: string): Promise<OnlineOrder[]> {
    return this.items.filter(item => 
      item.store_id === storeId && 
      (item.status === OrderStatus.CONFIRMED || item.status === OrderStatus.PREPARING)
    );
  }
  ...
  async getTotalSalesValue(storeId: string): Promise<number> {
    return this.items
      .filter(item => item.store_id === storeId && item.status === OrderStatus.CONFIRMED)
      .reduce((total, item) => total + item.total.value, 0);
  }
  ...
  async canCancelOrder(orderId: OnlineOrderId): Promise<boolean> {
    ...
    return order ? ['pending', 'confirmed'].includes(order.status) : false;
  }
